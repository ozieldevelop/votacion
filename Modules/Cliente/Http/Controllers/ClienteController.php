...
class ClienteController extends Controller
{
    ...
    function almacenarfilecv(Request $request)
   {
    ...
    public function dashboard(Request $request)
    {
                ...
                //dd($pactivo);
										return view('cliente::tablero')->with('periodoactivo', $pactivo)->with('enlace', $request->all() )->with('tienefoto', $tienefoto)->with('tipo', $tipo)->with('foto', $foto)->with('aspirantes', $listadoaspirantes)->with('nombreevento', trim($results2[0]["nombre"]))->with('tipoevent', $results2[0]["tipo"] )->with('id_evento', $idevendesc )->with('ideven', $idevendesc )->with('max_votos', $results2[0]["maxvotos"] )->with('f_inicia', $results2[0]["rangofecha1"] )->with('f_termina', $results2[0]["rangofecha2"] )->with('num_cliente', $cldoc )->with('ocupacion', trim($datoscliente[0]->ocupacion) )->with('profesion', trim($datoscliente[0]->profesion) )->with('trato', trim($datoscliente[0]->trato) )->with('nombre', trim($datoscliente[0]->nombre) )->with('agencia', trim($datoscliente[0]->agencia) );	 										
									//}
								/*}
								else
								{
        ...
        {                        
            $xdato2 = DB::select("select *  from directivos where num_cliente=".$osi->num_cliente."");
              ...
              <img src="https://portal.cooprofesionales.com.pa/mercadeo/files/333f41_newlogo1.png" style="width: 970px;">
							<br/> <label style="font-size:20px;color:#202020;font-style: italic;">'.  $time. '; '.trim($datoscliente[0]->trato).' '.trim($datoscliente[0]->nombre).'. <br/> Esta es la confirmaci&oacute;n de registro para:  '.trim($results2[0]["nombre"]).' 
     ...
     public function alldatadirectivos(Request $request)
    {
           ...
           $data = vtaspiranteModel::select(['id_evento','id_delegado','trato','num_cliente','nombre','apellido','ocupacion','profesion','img_delegado','estado','user_audit','fecha_aud','foto','tipo','memoria'])->where('id_evento',$evento)->get();
      ...
      public function cargaadjuntosScreenListar(Request $request)
    {
        try {
              ...
              ->where('cldoc', '=', 0 )->where('id_evento', '=', $id_evento)->where('eliminado', '=', 0)->get();
      ...
      function getDownload($adjuntoviewid)
    {
      try
        ...
        $headers = array(
              'Content-Type: '.$data[0]->tipoarchivo,
            );
