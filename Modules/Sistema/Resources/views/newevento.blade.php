...
<div class="row ">
   <div class="col-md-3">
      ...
      <div class="row"  id="votacionActivodiv">
         <div class="form-group col-sm-12 col-md-12 col-lg-12 ">
            <label for="inputflimitevotacion">Estatus Votaci&oacute;n</label>
            <input type="checkbox" class="form-control col-sm-12 col-md-12 col-lg-12 " id="votacionActivo"  name="votacionActivo">
         </div>
      </div>
      ...
      <div class="row"  id="lafechalimite">
         <div class="form-group col-sm-12 col-md-12 col-lg-12 ">
            <label for="inputflimite">Estatus PreInscripci&oacute;n</label>
            <input type="checkbox" class="form-control col-sm-12 col-md-12 col-lg-12 " id="preinscripActivo" name="preinscripActivo">
         </div>
      </div>
      <div class="row ">
         <div class="form-group col-sm-12 col-md-12 col-lg-12 ">
      ...
      <div class="row">
         <table id="lsteventos" class="display" cellspacing="0" style="width:100%">
            <thead>
               <tr>
                  <th>Nombre</th>
                  <th>Estatus Votaci&oacute;n </th>
                  <th>Estatus Preinscripci&oacute;n </th>
                  <th>Opci&oacute;n</th>
               </tr>
            </thead>
            <tbody>
...
<script>
   ...
   //var preinscripActivo = $('#preinscripActivo').val();
   
   //var votacionActivo = $('#votacionActivo').val();
   
   var preinscripActivo = $('#preinscripActivo').prop('checked');
   var votacionActivo = $('#votacionActivo').prop('checked');  
 


   var  votacionActivoval =0;
           if(votacionActivo == true)
           {    
                 votacionActivoval = 1;
           }
           
    var preinscripActivoval=0; 
    if(preinscripActivo == true)
           {    
                 preinscripActivoval = 1;
           }
  
   //console.log(votacionActivoval +'  '+ preinscripActivoval);
   ...
   */				
   ...
   /*if(seccselect=="2" )
   {
       if(preinscripActivo=="" || preinscripActivo==undefined || preinscripActivo.length < 0)
       {
   ...
   else if(votacionActivo=="" || votacionActivo==undefined || ran02.length < 0)
   {
   ...
   //preinscripActivo = preinscripActivo.replace(/\//g, "-")+ ':00';
   
   $.ajax({
   url: '{{ url("sistema/agregarnuevoevento")}}',
   data: {"nombre": nom01 ,"maxvotos": maxvotos ,"votacionActivo": votacionActivoval  ,"preinscripActivo": preinscripActivoval ,'tipo': secciones,'idzoom':idzoom, 'capitulos': JSON.stringify(seleccioncheckbox),'estadosasoc': JSON.stringify(seleccioncheckbox2)},
   method: 'post',
   headers: {
   	  ...
   	  var t = $('#lsteventos').DataTable({
   		...
   		ajax: '{{ url("sistema/cargareventos")}}',
   		  columns: [
   		   ...
   		   { data: 'votacionActivo', name: 'votacionActivo' , class: 'text-center'},
   		   { data: 'preinscripActivo', name: 'preinscripActivo', class: 'text-center' },
   ...
   data: {"evento": dato },
         success: function(result){
           ...
           if(datoz[0]['preinscripActivo']=="0")
           {    
                 $('#preinscripActivo').prop('checked', false);
           }
           else
             {
                 $('#preinscripActivo').prop('checked', true);
             }
           
          if(datoz[0]['votacionActivo']=="0")
          {    
             $('#votacionActivo').prop('checked', false);
          }
          else
          {
             $('#votacionActivo').prop('checked', true);
          }    
           ...
           if(tipo==2)
           {
               $('#maxperson').css('display','none');	
               //$('#lafechalimite').css('display','block');	
           }
           else
           {
             $('#maxperson').css('display','block');	
          ...
          // $('#preinscripActivo').val(preinscripActivo);
   		  $('#maxvotos').val(mvotos);
   ...
   */
   ...
   lobibox_emergente('info','top right',true,'Debe ingresar una fecha final.');
     ...
     //alert(preinscripActivo);
   $.ajax
   ({
    url: '{{ url("sistema/actualizarevento")}}',
    data: {"id": id_evento ,"votacionActivo": votacionActivoval ,"preinscripActivo": preinscripActivoval,"nombre": nombre ,"maxvotos": maxvotos ,'tipo': secciones,  'idzoom':idzoom, 'capitulos': JSON.stringify(seleccioncheckbox),'estadosasoc': JSON.stringify(seleccioncheckbox2)},
    method: 'post',
    headers: {
   	'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
   },
   ...
   $('#rangofecha1').val('');
   $('#rangofecha2').val('');
   $('#preinscripActivo').val('');
   $('#votacionActivo').val(''); 
   $('#id_zoom').val();
   
   /*
     $('#preinscripActivo').on('change', function() {
       model.preinscripActivo = (this.value);
     })
   */
...
<script>
   ...
   startDate:	anio+'/03/05'
     ...
     /*
   $('#preinscripActivo').datetimepicker({
   dayOfWeekStart : 1,
   lang:'en',
   //disabledDates:[anio+'/01/08',anio+'/01/09',anio+'/01/10'],
   startDate:	anio+'/03/05'
