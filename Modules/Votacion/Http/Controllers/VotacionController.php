...
class VotacionController extends Controller
{
    ...
    public function test1(Request $request)
    {
        try
        {
           ...
           $results2 = eventoModel::select(['id', 'nombre','votacionActivo', 'maxvotos', 'capitulos', 'estadosasoc', 'status', 'tipo'])->where('id', $idevendesc)->where('status', 1)->get();        
           $configuraciones = DB::select('select modo,correopruebas from conf');
          
            $results = eventoModel::select(['id', 'nombre',  'maxvotos', 'capitulos', 'estadosasoc', 'status', 'tipo'])->where('id', $idevendesc)->get();
        ...
        try
        {
            ...
            if ($cldoc > 0 && $idevendesc > 0)
            {
                ...
                if (count($results) > 0)
                {
                    ...
                    $results2 = eventoModel::select(['id', 'nombre', 'votacionActivo', 'maxvotos', 'capitulos', 'estadosasoc', 'status', 'tipo'])->where('id', $idevendesc)->where('status', 1)
                        ->get();
    ...
    public function finalizada(Request $request)
    {
        try
        {
          ...
          */
                                  ...
                                  INNER JOIN votantes AS b ON e.idvotante = b.id
                                  INNER JOIN conf_areas AS c ON e.id_area = c.id_area
                                  WHERE cast(aes_decrypt(`b`.`asociado`,'xyz123') as char charset utf8mb4) = " . $usuario . " 
                                  AND `e`.`id_evento` = " . $id_evento . " 
                                  GROUP BY `e`.`id_area`,`c`.`area_etiqueta`
                                  ORDER BY `e`.`id_area`,`c`.`area_etiqueta` ASC");
            ...
            $mensaje .= "<div class='col-xs-4 text-center' style='vertical-align: middle;'></div>";
                                    ...
                                    return view('votacion::datosvotacion')->with('mensaje', $mensaje)->with('nombre', $registrosenvio->NOMBRE )
                                        ->with('ideven', $id_evento)->with('areas', $areas)->with('detalles', $detalles);
